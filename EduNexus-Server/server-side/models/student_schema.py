...
class Query(db.Model):
    ...
    date_search = db.Column(db.DateTime, nullable=False,
...
class OngoingModule(db.Model):
    ...
    module = db.relationship(
        'Module', back_populates='onmodule_user_association')
...
class CompletedModule(db.Model):
    ...
    module = db.relationship(
        'Module', back_populates='module_comp_association')
...
class User(db.Model):
    ...
    user_module_association = db.relationship(
        'CompletedModule', back_populates='user')
    completed_modules = association_proxy('user_module_association', 'module')

    user_onmodule_association = db.relationship(
        'OngoingModule', back_populates='user')
    ongoing_modules = association_proxy('user_onmodule_association', 'module')

    user_topic_association = db.relationship(
        'CompletedTopics', back_populates='user')
...
class ProjectsStudent(db.Model):
    ...
    github_id = db.Column(db.String(50), db.ForeignKey(
        'user.github_id'), nullable=False)
...
class Topic(db.Model):
    ...
    topic_comp_association = db.relationship(
        'CompletedTopics', back_populates='topic')
...
class Module(db.Model):
    ...
    module_comp_association = db.relationship(
        'CompletedModule', back_populates='module')
    completed_by = association_proxy('module_comp_association', 'user')

    onmodule_user_association = db.relationship(
        'OngoingModule', back_populates='module')
